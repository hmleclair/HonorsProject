...
#---------------------------------------------------------------------------------------------
shoe_model = load_model('shoe_model.h5')
...
preds = shoe_model.predict_classes(x)
prob = shoe_model.predict_proba(x)
...
if preds[0][0] == 0:
        print('\tClass: High Heels')
	matches = []
	distances = []
	closest_match = []
	for i in range(1, 6):
		new_img = image_utils.load_img('images_shoes/validation/heels/shoe_test_' + `i`+ '.jpg', target_size=(150, 150))
	...
	matches = []
        ...
        closest_match = []
        for i in range(6, 11):
                new_img = image_utils.load_img('images_shoes/validation/sneakers/shoe_test_' + `i`+ '.jpg', target_size=(150, 150))
                matches.append(new_img)
                distances.append(euclidean_distance(img_to_array(new_img), img_to_array(img), len(img_to_array(new_img))))
        distances = np.array(distances)
        closest_match = distances.argmin()
